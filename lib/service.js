...
Service.prototype.$call = function(name, args) {
  ...
  return new Promise(function edboHandler(resolve, reject) {
    ...
    // bad style
    args.Id_Language = 1;
    args.ActualDate = self.$formatDate(new Date());
    ...
    self.$client[name](args, function(err, data) {
      if(err) {
        ...
        console.error('');
        console.error('EDBO connection error: "' + name + '"');
      ...
      self.GetLastError()
        .then(function(err) {
          if(err.dLastError[0].LastErrorCode === 101) { // session blocked
            ...
            function() {
              reject(err.dLastError[0]);
            });
          }
          else if(err.dLastError[0].LastErrorCode === 0) {
            edboHandler(resolve, reject);
          }
          else {
...
Service.prototype.$formatDate = function(date) {
  function padding(n) {
    return n < 10 ? '0' + n : n;
  }
