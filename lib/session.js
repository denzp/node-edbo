...
function Session(options, callback) {
  ...
  this.check_cache_(this.client.host_url, this.client.guides_path, function(url) {
    soap.createClient(url, function(err, guides_client) {
  ...
  this.check_cache_(this.client.host_url, this.client.person_path, function(url) {
    soap.createClient(url, function(err, person_client) {
...
Session.prototype.check_cache_ = function(origin_host, origin_path, callback) {
  var origin_url = origin_host + origin_path + "?WSDL";
  var cache_folder = process.cwd() + "/wsdl_cache/";
  var cache_path = cache_folder + ((origin_host + origin_path).replace(/\W/gi, "_"));

  var try_load_file_ = function(callback, neasted) {
    http
    .get(origin_url, function(res) {
      var Data = "";
  ...
  //get remote url -- workaround to bugs with empty response
  http.get(origin_host + origin_path, function() {
    fs.stat(cache_path, function(err) {
      if(err) {
        try_load_file_(callback);
        return;
      }
