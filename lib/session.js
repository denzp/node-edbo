...
Session.prototype.request = function(service_name, method_name, args, callback) {
  if(!this.services[service_name])
    throw new EDBOError({ code: -1, msg: "not connected to service '" + service_name + "'" });

  if(!this.services[service_name].hasOwnProperty(method_name))
    throw new EDBOError({ code: -1, msg: "service '" + service_name + "' doesn't have method '" + method_name + "'" });
  ...
  this.services[service_name][method_name](args, function(err, result) {
    ...
    if(fields.length > 1)
      throw new EDBOError({ code: -1, msg: "TODO! fields.length > 1" });
...
Session.prototype.getLastError = function() {
  //TODO
  return true;
}
